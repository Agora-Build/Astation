...
namespace agora
{
    namespace tools
    {
                                    ...
                                    const std::string &message, uint32_t signSize)
        {
            ...
            CCHmac(kCCHmacAlgSHA1, appCertificate.data(), appCertificate.length(),
                   message.data(), message.length(), md);
                                     ...
                                     const std::string &message, uint32_t signSize)
        {
            ...
            CCHmac(kCCHmacAlgSHA256, appCertificate.data(), appCertificate.length(),
                   message.data(), message.length(), md);
            return std::string(reinterpret_cast<char *>(md), signSize);
#else
            unsigned char md[EVP_MAX_MD_SIZE];
